...
function readList(body: { name?: string; description?: string; seoTitle?: string; seoDescription?: string; status?: string; items?: ListTitle[] }) {
  ...
  return { name, description, items, status: body.status === "published" ? "published" as const : "draft" as const, seoTitle: body.seoTitle?.trim().slice(0, 70) || null, seoDescription: body.seoDescription?.trim().slice(0, 170) || null };
}
...
export async function PATCH(request: Request) {
  ...
  const member = await adminMember();
  if (!member) return Response.json({ error: "Studio access required." }, { status: 403 });
  const body = await request.json() as { id?: string; name?: string; description?: string; seoTitle?: string; seoDescription?: string; status?: string; items?: ListTitle[] };
  ...
  await db.transaction(async (tx) => {
    await tx.update(editorLists).set({ name: content.name, description: content.description, seoTitle: content.seoTitle, seoDescription: content.seoDescription, status: content.status, publishedAt: content.status === "published" ? existing.publishedAt ?? new Date() : null, updatedAt: new Date() }).where(eq(editorLists.id, id));
