...
export async function PublicTitlePage({ type, slug }: { type: SeoTitleType; slug: string }) {
  const title = await getSeoTitle(type, slug);
  if (!title) notFound();
  const appTitleHref = `/?titleId=${title.id}&titleType=${type}`;
  ...
  return <main className="public-title-page">
    ...
    <section className="public-title-about"><p className="eyebrow">WHAT IS CINEAPE?</p><h2>Find your next great watch through people you trust.</h2><p>Rate what you watch, build a Watchlist, and share the titles you love with friends and family.</p><a href={appTitleHref}>Create your free CineApe account</a></section>
