...
function CirclePage({ onInvite, onOpen }: { onInvite: () => void; onOpen: (title?: string, meta?: string, score?: string, tmdbId?: number, type?: "movie" | "tv") => void }) { return <><CompactPeopleFinder/><CompactCirclePageV2 onInvite={onInvite} onOpen={onOpen}/><MovieNightPanel/></>; }
...
type GroupSharedPick = { id: string; titleId: string; tmdbId: number; title: string; type: "movie" | "tv"; year: number | null; posterPath: string | null; addedBy: { id: string; displayName: string; avatarUrl: string | null }; savedCount: number; savedByViewer: boolean };
...
type LiveRecommendation = { id: string; titleId: string; status: "pending" | "watching" | "watched" | "not_interested"; note: string | null; title: string; type: "movie" | "tv"; year: number | null; posterPath: string | null; person: { displayName: string; avatarUrl: string | null } | null };
...
function ForYouTrackingPage({ onInvite, onOpen }: { onInvite: () => void; onOpen: (title?: string, meta?: string, score?: string) => void }) {
  ...
  return <section className="page live-inbox tracking-inbox">
    ...
    {loading ? <div className="panel inbox-loading">Loading your list…</div> : personalList ? library.length ? <div className="panel inbox live-inbox-list personal-library-list tracking-library">{library.map(libraryRow)}</div> : <div className="panel inbox-empty"><div><b>{emptyText}</b><p>Open a title and add it to this personal list whenever you want to come back to it.</p></div></div> : displayedRecommendations.length ? <div className="panel inbox live-inbox-list">{displayedRecommendations.map(item => <article key={item.id}><button className="inbox-cover" onClick={() => onOpen(item.title, `${item.year ?? "—"} · ${item.type === "tv" ? "TV series" : "Movie"}`, "—")} aria-label={`Open ${item.title}`}>{item.posterPath && <img src={item.posterPath} alt="" />}</button><div><h3>{item.title}</h3><p>{view === "sent" ? "Sent to" : "From"} <b>{view === "sent" ? sentRecipients(item) : item.person?.displayName ?? "a CineApe member"}</b> · {item.type === "tv" ? "TV series" : "Movie"}{item.year ? ` · ${item.year}` : ""}</p>{item.note && <em>“{item.note}”</em>}</div>{view === "received" ? <div>{item.status === "pending" && <button className="small-primary" onClick={() => void updateRecommendation(item.id, "watching")}>Start watching</button>}{item.status === "watching" && <button className="small-primary" onClick={() => void updateRecommendation(item.id, "watched")}>Mark watched</button>}</div> : <strong className="rec-status">{view === "sent" ? sentStatus(item) : item.status === "watched" ? "Watched" : "Sent"}</strong>}</article>)}</div> : <div className="panel inbox-empty"><div><b>{emptyText}</b><p>Invite your people and share your first recommendation when you find a title they will love.</p></div><button className="primary" onClick={onInvite}>Invite people</button></div>}
  </section>;
}
