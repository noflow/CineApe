...
type TitleReturn = { page: Exclude<Page, "Title">; scrollY: number };
...
export default function Home() {
  ...
  const [titleReturn, setTitleReturn] = useState<TitleReturn | null>(null);
  ...
  const openTitle = (title = "Mickey 17", meta = "2025 · Science fiction", score = "8.2", tmdbId?: number, type?: "movie" | "tv") => {
    if (page !== "Title") setTitleReturn({ page, scrollY: window.scrollY });
    setSelectedTitle({ title, meta, score, tmdbId, type });
    setPage("Title");
  };
  const returnFromTitle = () => {
    const destination = titleReturn ?? { page: "Discover" as const, scrollY: 0 };
    setPage(destination.page);
    window.requestAnimationFrame(() => window.requestAnimationFrame(() => window.scrollTo(0, destination.scrollY)));
  };
  ...
  return <div className="app-shell">
    ...
    {page === "Title" && <TitleDetails selection={selectedTitle} onBack={returnFromTitle} backLabel={`Back to ${titleReturn?.page ?? "Discover"}`} onOpenTitle={openTitle} onRecommend={(title) => chooseShareTitle(title, "recommend")} onAddToGroup={(title) => chooseShareTitle(title, "groupPick")}/>} 
...
type AppNotification = { id: string; kind: "recommendation" | "group_join" | "streaming" | "episode" | "release" | "chat" | "friend_request"; message: string; createdAt: string; readAt: string | null };
...
function NotificationPanel({ onClose }: { onClose: () => void }) {
  ...
  const icon = (kind: AppNotification["kind"]) => kind === "recommendation" ? "✦" : kind === "group_join" ? "♧" : kind === "chat" ? "✉" : kind === "friend_request" ? "+" : kind === "episode" ? "◉" : kind === "release" ? "✦" : "▶";
...
type EditorReview = { id: string; slug: string; headline: string; body: string; score: number; seoTitle: string | null; seoDescription: string | null; status: "draft" | "published" | "archived"; tmdbId: number; title: string; type: "movie" | "tv"; year: number | null; posterPath: string | null; createdAt: string; publishedAt: string | null; scheduledAt: string | null };
...
function StudioEditorialLegacy() {
  ...
  const [headline, setHeadline] = useState(""); const [body, setBody] = useState(""); const [score, setScore] = useState(8); const [seoTitle, setSeoTitle] = useState(""); const [seoDescription, setSeoDescription] = useState(""); const [status, setStatus] = useState<"draft" | "published" | "archived">("draft");
...
function StudioEditorial() {
  ...
  const loadReviews = () => void fetch("/api/admin/editor-reviews").then(response => response.ok ? response.json() as Promise<{ reviews?: EditorReview[] }> : null).then(data => setReviews(data?.reviews ?? [])).catch(() => setReviews([]));
  useEffect(() => { loadReviews(); }, []);
  useEffect(() => { setScheduledAt(editing?.scheduledAt ? new Date(editing.scheduledAt).toISOString().slice(0, 16) : ""); }, [editing]);
  const clearEditor = () => { setEditing(null); setSelected(null); setTitleQuery(""); setMatches([]); setHeadline(""); setBody(""); setScore(8); setSeoTitle(""); setSeoDescription(""); setStatus("draft"); setScheduledAt(""); setMessage(""); };
  ...
  const suggestedSeoTitle = selected ? `${selected.title} review: worth watching? | CineApe` : "Your review title | CineApe";
  const seoChecks = [
    { label: "Title selected", ready: Boolean(selected) },
    { label: "Memorable headline", ready: headline.trim().length >= 12 },
    { label: `Detailed original review (${wordCount} words)`, ready: wordCount >= 250 },
    { label: `SEO title (${seoTitle.length}/70)`, ready: seoTitle.trim().length >= 25 && seoTitle.length <= 70 },
    { label: `Meta description (${seoDescription.length}/170)`, ready: seoDescription.trim().length >= 100 && seoDescription.length <= 170 },
  ];
  const seoReady = seoChecks.filter(check => check.ready).length;
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (!selected || saving) { setMessage("Choose a title first."); return; } setSaving(true); setMessage(""); const payload = editing ? { id: editing.id, headline, body, score, seoTitle, seoDescription, status, scheduledAt: scheduledAt || null } : { tmdbId: selected.id, type: selected.type, name: selected.title, year: selected.year ? Number(selected.year) : null, posterPath: selected.image, headline, body, score, seoTitle, seoDescription, status, scheduledAt: scheduledAt || null }; const response = await fetch("/api/admin/editor-reviews", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const data = await response.json() as { error?: string }; if (!response.ok) { setMessage(data.error ?? "Review could not be saved."); setSaving(false); return; } const wasEditing = Boolean(editing); clearEditor(); setMessage(wasEditing ? "Review updated." : status === "published" ? "Official review published." : "Draft saved in Studio."); setSaving(false); loadReviews(); };
  return <section className="studio-editorial">
    <div className="studio-section-head"><div><p className="eyebrow">EDITORIAL DESK</p><h2>Official CineApe reviews</h2><p>Write your real take, make the search preview clear, then publish a page worth finding.</p></div></div>
    <div className="studio-editor-grid">
      <form className="panel editorial-form" onSubmit={submit}>
        {editing && <div className="editor-editing"><b>Editing: {editing.title}</b><button type="button" onClick={clearEditor}>New review</button></div>}
        <label>FIND A TITLE<div className="editor-title-search"><input value={titleQuery} onChange={event => setTitleQuery(event.target.value)} placeholder="Search movie or TV show" disabled={Boolean(editing)} /><button type="button" className="secondary" onClick={() => void findTitle()} disabled={Boolean(editing)}>Find</button></div></label>
        {matches.length > 0 && <div className="editor-matches">{matches.map(match => <button type="button" key={`${match.type}-${match.id}`} onClick={() => { setSelected(match); setMatches([]); }}><div><b>{match.title}</b><small>{match.subtitle}</small></div></button>)}</div>}
        {selected && <div className="editor-selected">{selected.image && <img src={selected.image} alt=""/>}<b>{selected.title}<small>{selected.subtitle}</small></b>{!editing && <button type="button" onClick={() => setSelected(null)}>×</button>}</div>}
        <label>REVIEW HEADLINE<input value={headline} onChange={event => setHeadline(event.target.value)} maxLength={180} placeholder="Your short, memorable verdict" /></label>
        <label>CINEAPE SCORE <span>{score}/10</span><input type="range" min="1" max="10" value={score} onChange={event => setScore(Number(event.target.value))}/></label>
        <label>YOUR REVIEW <span>{wordCount} words</span><textarea value={body} onChange={event => setBody(event.target.value)} maxLength={12000} placeholder="Explain who will enjoy it, what works, and what does not—without spoilers."/></label>
        <fieldset><legend>SEARCH PREVIEW <small>How this can appear in Google</small></legend><label>SEO TITLE<input value={seoTitle} onChange={event => setSeoTitle(event.target.value)} maxLength={70} placeholder={suggestedSeoTitle}/></label><label>META DESCRIPTION<textarea value={seoDescription} onChange={event => setSeoDescription(event.target.value)} maxLength={170} placeholder="A clear, spoiler-safe reason to read this review."/></label><div className="editorial-search-preview"><span>cineape.com › reviews</span><b>{seoTitle.trim() || suggestedSeoTitle}</b><p>{seoDescription.trim() || "Write a useful description so people know why this CineApe review is worth reading."}</p></div></fieldset>
        <section className="editorial-seo-checklist" aria-label="SEO publishing checklist"><div><b>SEO checklist</b><span>{seoReady}/{seoChecks.length} ready</span></div>{seoChecks.map(check => <p className={check.ready ? "ready" : ""} key={check.label}><i>{check.ready ? "✓" : "○"}</i>{check.label}</p>)}</section>
        <label>GO LIVE ON <small>Optional — leave empty to publish immediately</small><input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} disabled={status !== "published"} /></label>
        <label>PUBLISHING STATUS<select value={status} onChange={event => setStatus(event.target.value as "draft" | "published" | "archived")}><option value="draft">Save as draft</option><option value="published">Publish now or schedule</option><option value="archived">Archive (remove from public pages)</option></select></label>
        <button className="primary wide" disabled={saving}>{saving ? "Saving…" : editing ? "Save changes" : status === "published" ? "Publish official review" : "Save draft"}</button>{message && <p className="editor-message">{message}</p>}
      </form>
      <div className="studio-review-list"><div className="studio-review-list-head"><h3>Your reviews</h3><a href="/reviews" target="_blank" rel="noreferrer">Open public hub ↗</a></div>{reviews.length ? reviews.map(review => <article className="panel" key={review.id}>{review.posterPath ? <img src={review.posterPath} alt=""/> : <span>★</span>}<div><b>{review.title}</b><small>{review.headline}</small><em>{review.status === "archived" ? "Archived" : review.status === "published" ? (review.scheduledAt && new Date(review.scheduledAt) > new Date() ? "Scheduled" : "Published") : "Draft"} · {review.score}/10</em>{review.status === "published" && <a className="studio-live-link" href={`/reviews/${review.slug}`} target="_blank" rel="noreferrer">View live review ↗</a>}<button type="button" className="studio-edit-review" onClick={() => chooseReview(review)}>Edit review</button></div></article>) : <div className="panel studio-empty">Your official CineApe reviews will appear here.</div>}</div>
    </div>
  </section>;
}
...
type EditableListItem = { tmdbId: number; type: "movie" | "tv"; name: string; year: number | null; posterPath: string | null; position: number };
type EditableList = { id: string; name: string; description: string; slug: string; status: "draft" | "published" | "archived"; seoTitle: string | null; seoDescription: string | null; scheduledAt: string | null; items: EditableListItem[] };

function StudioListsV2() {
  const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [seoTitle, setSeoTitle] = useState(""); const [seoDescription, setSeoDescription] = useState(""); const [status, setStatus] = useState<"draft" | "published" | "archived">("draft"); const [scheduledAt, setScheduledAt] = useState("");
  const [query, setQuery] = useState(""); const [matches, setMatches] = useState<EditorialTitle[]>([]); const [items, setItems] = useState<EditorialTitle[]>([]); const [lists, setLists] = useState<EditableList[]>([]); const [editing, setEditing] = useState<EditableList | null>(null); const [message, setMessage] = useState(""); const [saving, setSaving] = useState(false);
  const loadLists = () => void fetch("/api/admin/editor-lists").then(response => response.ok ? response.json() as Promise<{ lists?: EditableList[] }> : null).then(data => setLists(data?.lists ?? [])).catch(() => setLists([]));
  useEffect(() => { loadLists(); }, []);
  useEffect(() => { setScheduledAt(editing?.scheduledAt ? new Date(editing.scheduledAt).toISOString().slice(0, 16) : ""); }, [editing]);
  const clearEditor = () => { setEditing(null); setName(""); setDescription(""); setSeoTitle(""); setSeoDescription(""); setStatus("draft"); setScheduledAt(""); setItems([]); setMatches([]); setQuery(""); setMessage(""); };
  const find = async () => { if (query.trim().length < 2) { setMessage("Type at least two letters to find a title."); return; } const response = await fetch(`/api/tmdb?mode=search&query=${encodeURIComponent(query.trim())}`); const data = response.ok ? await response.json() as { results?: EditorialTitle[] } : null; setMatches((data?.results ?? []).filter(item => item.type === "movie" || item.type === "tv")); };
  const add = (title: EditorialTitle) => { if (!items.some(item => item.id === title.id && item.type === title.type)) setItems(current => [...current, title]); setMatches([]); setQuery(""); };
  const edit = (list: EditableList) => { setEditing(list); setName(list.name); setDescription(list.description); setSeoTitle(list.seoTitle ?? ""); setSeoDescription(list.seoDescription ?? ""); setStatus(list.status); setItems([...list.items].sort((a, b) => a.position - b.position).map(item => ({ id: item.tmdbId, type: item.type, title: item.name, year: item.year ? String(item.year) : null, image: item.posterPath, subtitle: `${item.year ?? "—"} · ${item.type === "tv" ? "TV series" : "Movie"}` }))); setMessage(`Editing ${list.name}.`); };
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); if (saving) return; setSaving(true); const payload = { name, description, seoTitle, seoDescription, status, scheduledAt: scheduledAt || null, items: items.map(item => ({ tmdbId: item.id, type: item.type, name: item.title, year: item.year ? Number(item.year) : null, posterPath: item.image })) }; const response = await fetch("/api/admin/editor-lists", { method: editing ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing ? { ...payload, id: editing.id } : payload) }); const data = await response.json() as { error?: string }; if (!response.ok) { setMessage(data.error ?? "List could not be saved."); setSaving(false); return; } const edited = Boolean(editing); clearEditor(); setMessage(edited ? (status === "published" ? "Published list updated." : "List moved back to drafts.") : (status === "published" ? "Must-watch list published." : "List saved as a draft.")); setSaving(false); loadLists(); };
  return <section className="studio-editorial"><div className="studio-section-head"><div><p className="eyebrow">CINEAPE EDITORIAL</p><h2>{editing ? "Refine your must-watch list" : "Build a must-watch list"}</h2><p>Published lists appear at <b>/must-watch</b> and are ready to be found and shared.</p></div></div><div className="studio-editor-grid"><form className="panel editorial-form" onSubmit={submit}>{editing && <div className="editor-editing"><b>Editing: {editing.name}</b><button type="button" onClick={clearEditor}>New list</button></div>}<label>LIST NAME<input value={name} onChange={event => setName(event.target.value)} maxLength={160} placeholder="e.g. 10 TV shows worth starting tonight" /></label><label>INTRODUCTION<textarea value={description} onChange={event => setDescription(event.target.value)} maxLength={1000} placeholder="Tell people why this list is worth their time." /></label><label>ADD TITLES<div className="editor-title-search"><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search a movie or show" /><button type="button" className="secondary" onClick={() => void find()}>Find</button></div></label>{matches.length > 0 && <div className="editor-matches">{matches.map(match => <button type="button" key={`${match.type}-${match.id}`} onClick={() => add(match)}>{match.image ? <img src={match.image} alt="" /> : <span>◉</span>}<div><b>{match.title}</b><small>{match.subtitle}</small></div></button>)}</div>}<div className="list-picked-titles">{items.length ? items.map((item, index) => <article key={`${item.type}-${item.id}`}>{item.image ? <img src={item.image} alt="" /> : <span>{index + 1}</span>}<b>{index + 1}. {item.title}</b><div className="list-item-actions"><button type="button" aria-label={`Move ${item.title} up`} disabled={index === 0} onClick={() => move(index, -1)}>↑</button><button type="button" aria-label={`Move ${item.title} down`} disabled={index === items.length - 1} onClick={() => move(index, 1)}>↓</button><button type="button" aria-label={`Remove ${item.title}`} onClick={() => setItems(current => current.filter(choice => choice.id !== item.id || choice.type !== item.type))}>×</button></div></article>) : <p>Add at least three movies or shows.</p>}</div><fieldset><legend>SEARCH PREVIEW <small>Optional, but recommended</small></legend><label>SEO TITLE<input value={seoTitle} onChange={event => setSeoTitle(event.target.value)} maxLength={70} /></label><label>META DESCRIPTION<textarea value={seoDescription} onChange={event => setSeoDescription(event.target.value)} maxLength={170} /></label></fieldset><label>GO LIVE ON <small>Optional — leave empty to publish immediately</small><input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} disabled={status !== "published"} /></label><label>PUBLISHING STATUS<select value={status} onChange={event => setStatus(event.target.value as "draft" | "published" | "archived")}><option value="draft">Save as draft</option><option value="published">Publish now or schedule</option><option value="archived">Archive (remove from public pages)</option></select></label><button className="primary wide" disabled={saving}>{saving ? "Saving…" : editing ? "Save list changes" : status === "published" ? "Publish must-watch list" : "Save draft"}</button>{message && <p className="editor-message">{message}</p>}</form><div className="studio-review-list"><div className="studio-review-list-head"><h3>Your lists</h3><a href="/must-watch" target="_blank" rel="noreferrer">Open public hub ↗</a></div>{lists.length ? lists.map(list => <article className="panel" key={list.id}><span>☰</span><div><b>{list.name}</b><small>{list.status === "archived" ? "Archived" : list.status === "published" ? (list.scheduledAt && new Date(list.scheduledAt) > new Date() ? "Scheduled" : "Published") : "Draft"}</small><em>cineape.com/must-watch/{list.slug}</em>{list.status === "published" && <a className="studio-live-link" href={`/must-watch/${list.slug}`} target="_blank" rel="noreferrer">View live list ↗</a>}<button type="button" className="studio-edit-review" onClick={() => edit(list)}>Edit list</button></div></article>) : <div className="panel studio-empty">Your CineApe lists will appear here.</div>}</div></div></section>;
}
...
type Achievement = { id: string; icon: string; name: string; description: string; value: number; target: number; progress: number; earned: boolean };

function AchievementShelf({ achievements }: { achievements: Achievement[] }) {
  const earned = achievements.filter(item => item.earned).length;
  return <section className="achievement-shelf" aria-label="Your achievements"><div className="achievement-shelf-head"><div><p className="eyebrow">YOUR ACHIEVEMENTS</p><h2>Small wins, real movie memories.</h2></div><span>{earned}/{achievements.length} earned</span></div><div className="achievement-grid">{achievements.map(item => <article className={item.earned ? "earned" : ""} key={item.id}><i>{item.icon}</i><div><b>{item.name}</b><small>{item.description}</small><div className="achievement-progress"><span style={{ width: `${item.target ? (item.progress / item.target) * 100 : 0}%` }} /></div><em>{item.earned ? "Earned" : `${item.progress}/${item.target}`}</em></div></article>)}</div></section>;
}
...
function ProfilePage() {
  ...
  const load = async () => { await fetch("/api/account", { method: "POST" }); const response = await fetch("/api/account"); if (!response.ok) return; const data = await response.json() as { profile: MemberProfile; stats: { friends: number; ratings: number; sent: number }; achievements: Achievement[] }; setProfile(data.profile); setStats(data.stats); setAchievements(data.achievements ?? []); setDisplayName(data.profile.displayName); setBio(data.profile.bio ?? ""); setFriendListVisible(data.profile.friendListVisible ?? true); };
  useEffect(() => { void load(); }, []);
...
function DiscoverPage({ onOpen, resume, onSnapshot }: { onOpen: (title?: string, meta?: string, score?: string, tmdbId?: number, type?: "movie" | "tv") => void; resume: DiscoverResume | null; onSnapshot: (snapshot: DiscoverResume) => void }) {
  ...
  const subtitle = hasActiveYearRange ? `${filter === "tv" ? "Series" : filter === "movie" ? "Movies" : "Movies and series"} released ${activeYearFrom === activeYearTo ? `in ${activeYearFrom}` : `from ${activeYearFrom} to ${activeYearTo}`}, newest first with a proven audience.` : category === "upcoming" ? `Upcoming ${filter === "tv" ? "series" : "movies"} ordered by their nearest release date.` : category === "reality" ? "Reality TV with enough audience signal to be worth your time." : filter === "all" ? "Recent English-language movies and scripted series with a proven audience." : filter === "movie" ? "Recent movies with enough audience signal to be worth your next night in." : "Recent scripted series with enough audience signal for your next binge.";
...
type CircleGroup = { id: string; name: string; avatarUrl?: string | null; createdAt: string; memberCount: number; pickCount: number; isOwner: boolean; unreadMessages?: boolean };
...
function CompactCirclePageV2({ onInvite, onOpen }: { onInvite: () => void; onOpen: (title?: string, meta?: string, score?: string, tmdbId?: number, type?: "movie" | "tv") => void }) {
  ...
  return <section className="compact-circle">
    ...
    <section className="circle-section">
      ...
      {loading ? <div className="panel circle-loading">Loading your groups...</div> : groups.length ? <div className="group-grid live-group-grid sleek-group-grid">{groups.map((group, index) => <button className={`group-tile tone-${index % 3} ${group.unreadMessages ? "has-unread" : ""}`} key={group.id} onClick={() => setOpenGroup(group)} aria-label={`Open ${group.name}`}><span className="group-tile-avatar">{group.avatarUrl ? <img src={group.avatarUrl} alt="" /> : <span>{index % 3 === 0 ? "✦" : index % 3 === 1 ? "⌂" : "◉"}</span>}</span>{group.unreadMessages && <span className="group-tile-unread">New</span>}<span className="group-tile-copy"><b>{group.name}</b><small>{group.memberCount} {group.memberCount === 1 ? "member" : "members"}</small></span><span className="group-tile-arrow" aria-hidden="true">›</span></button>)}</div> : !creating && <div className="panel circle-empty"><div><b>Create a home for your next watch.</b><p>Start a private group for your family, friend group, or recurring movie night.</p></div><button className="primary" onClick={() => setCreating(true)}>Create a group</button></div>}
    </section>
    ...
    {openGroup && <GroupSpaceModalV2 group={openGroup} onClose={() => { setOpenGroup(null); setGroups(current => current.map(item => item.id === openGroup.id ? { ...item, unreadMessages: false } : item)); }} onInvite={() => { setOpenGroup(null); setInvitingGroup(openGroup); }} onOpen={onOpen}/>} 
  </section>;
}
...
type ChatMessage = { id: string; body: string; createdAt: string; sender: { id: string; displayName: string; avatarUrl: string | null }; title: { id: string; tmdbId: number; name: string; type: "movie" | "tv"; year: number | null; posterPath: string | null } | null; reactionCounts: Record<string, number>; viewerReaction: string | null };
type ChatChannel = { kind: "friend" | "group"; id: string; name: string; avatarUrl?: string | null };

function ChatMessageReactions({ message, onReact }: { message: ChatMessage; onReact: (emoji: string) => void }) {
  return <div className="chat-message-reactions" aria-label="Message reactions">{["👍", "😂", "🔥"].map(emoji => <button type="button" className={message.viewerReaction === emoji ? "chosen" : ""} key={emoji} onClick={() => onReact(emoji)}>{emoji}{message.reactionCounts[emoji] ? <small>{message.reactionCounts[emoji]}</small> : null}</button>)}</div>;
}
...
function GroupSpaceModalV2({ group, onClose, onInvite, onOpen }: { group: CircleGroup; onClose: () => void; onInvite: () => void; onOpen: (title?: string, meta?: string, score?: string, tmdbId?: number, type?: "movie" | "tv") => void }) {
  ...
  const [sharingTitleId, setSharingTitleId] = useState<string | null>(null);
  ...
  useEffect(() => { void load(); const interval = window.setInterval(() => void load(), 12000); return () => window.clearInterval(interval); }, [group.id]);
  useEffect(() => { messageEndRef.current?.scrollIntoView({ block: "end" }); }, [messages.length, loading]);
  ...
  const reactToMessage = async (message: ChatMessage, emoji: string) => {
    const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "react", messageId: message.id, emoji }) });
    const data = await response.json() as { reactionCounts?: Record<string, number>; viewerReaction?: string | null };
    if (response.ok) setMessages(current => current.map(item => item.id === message.id ? { ...item, reactionCounts: data.reactionCounts ?? item.reactionCounts, viewerReaction: data.viewerReaction ?? null } : item));
  };
  const sharePickInChat = async (pick: GroupSharedPick) => {
    if (sharingTitleId) return;
    setSharingTitleId(pick.id);
    const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ groupId: group.id, body: "Worth a look:", title: { tmdbId: pick.tmdbId, type: pick.type, name: pick.title, year: pick.year, posterPath: pick.posterPath } }) });
    if (response.ok) await load();
    setSharingTitleId(null);
  };
  const savePick = async (pick: GroupSharedPick) => {
    if (savingPick || pick.savedByViewer) return;
  ...
  return <div className="backdrop group-space-backdrop" onClick={onClose}><section className="modal group-space-modal group-space-v2" onClick={event => event.stopPropagation()}><button className="close" onClick={onClose}>×</button>
    ...
    <div className="group-space-grid"><section className="group-wall"><div className="group-space-section-head"><div><p className="eyebrow">GROUP WALL</p><h3>Talk it out</h3></div><span className={draft.trim() ? "group-chat-typing" : ""}>{draft.trim() ? "You are typing…" : "Private to this group"}</span></div><div className="group-wall-messages">{loading ? <p>Loading the wall...</p> : messages.length ? messages.map(message => <article key={message.id} className={message.sender.id === viewerId ? "mine" : ""}>{message.sender.id !== viewerId && <Avatar imageUrl={message.sender.avatarUrl}>{initials(message.sender.displayName)}</Avatar>}<div><b>{message.sender.id === viewerId ? "You" : message.sender.displayName}</b>{message.body && <p>{message.body}</p>}{message.title && <button type="button" className="chat-title-card" onClick={() => onOpen(message.title!.name, `${message.title!.year ?? "—"} · ${message.title!.type === "tv" ? "TV series" : "Movie"}`, "—", message.title!.tmdbId, message.title!.type)}>{message.title!.posterPath ? <img src={message.title!.posterPath} alt="" /> : <span>{message.title!.type === "tv" ? "TV" : "Movie"}</span>}<strong>{message.title!.name}<small>{message.title!.year ?? "—"} · {message.title!.type === "tv" ? "TV series" : "Movie"}</small></strong></button>}<time>{new Date(message.createdAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><ChatMessageReactions message={message} onReact={emoji => void reactToMessage(message, emoji)} /></div></article>) : <p>Start the conversation. Make the next watch a group decision.</p>}</div><div ref={messageEndRef} /><form className="group-wall-compose" onSubmit={send}><textarea value={draft} onChange={event => setDraft(event.target.value)} placeholder={`Message ${group.name}`} maxLength={2000}/><button className="primary" disabled={!draft.trim()}>Send</button></form></section>
      <section className="group-picks"><div className="group-space-section-head"><div><p className="eyebrow">SHARED LIST</p><h3>Movies & TV</h3></div><span>{picks.length} {picks.length === 1 ? "pick" : "picks"}</span></div><div className="tabs group-pick-tabs"><button className={filter === "all" ? "chosen" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "movie" ? "chosen" : ""} onClick={() => setFilter("movie")}>Movies</button><button className={filter === "tv" ? "chosen" : ""} onClick={() => setFilter("tv")}>TV</button></div><div className="group-pick-list">{loading ? <p>Loading shared picks...</p> : visiblePicks.length ? visiblePicks.map(pick => <article key={pick.id}><button className="group-pick-title" onClick={() => { onClose(); onOpen(pick.title, `${pick.year ?? "—"} · ${pick.type === "tv" ? "TV series" : "Movie"}`, "—", pick.tmdbId, pick.type); }}>{pick.posterPath ? <img src={pick.posterPath} alt=""/> : <span>{pick.type === "tv" ? "TV" : "Movie"}</span>}<div><b>{pick.title}</b><small>{pick.type === "tv" ? "TV series" : "Movie"}{pick.year ? ` · ${pick.year}` : ""}</small><em>Added by {pick.addedBy.displayName}</em><GroupPickReactions pick={pick} onReact={emoji => void reactToPick(pick, emoji)}/></div></button><div className="group-pick-actions"><button className={pick.savedByViewer ? "secondary saved-group-pick" : "small-primary"} disabled={Boolean(savingPick) || pick.savedByViewer} onClick={() => void savePick(pick)}>{savingPick === pick.id ? "Saving..." : pick.savedByViewer ? "Saved" : "Save"}</button><button type="button" className="group-share-pick" disabled={Boolean(sharingTitleId)} onClick={() => void sharePickInChat(pick)}>{sharingTitleId === pick.id ? "Sharing..." : "Chat"}</button></div></article>) : <p>No shared picks yet. Add one from any title page.</p>}</div></section></div>
...
function TitleDetailsLegacy({ selection, onBack, backLabel, onRecommend, onAddToGroup }: { selection: TitleSelection; onBack: () => void; backLabel: string; onRecommend: (title: ShareTitle) => void; onAddToGroup: (title: ShareTitle) => void }) {
...
function TitleDetails({ selection, onBack, backLabel, onOpenTitle, onRecommend, onAddToGroup }: { selection: TitleSelection; onBack: () => void; backLabel: string; onOpenTitle: (title: string, meta: string, score: string) => void; onRecommend: (title: ShareTitle) => void; onAddToGroup: (title: ShareTitle) => void }) {
  ...
  return <div className="cast-click-zone" onClick={onTitleClick}><TitleDetailsLegacy selection={selection} onBack={onBack} backLabel={backLabel} onRecommend={onRecommend} onAddToGroup={onAddToGroup}/>{castName && <CastFilmographyModal name={castName} onClose={() => setCastName(null)} onOpenTitle={onOpenTitle}/>} {mobileTrailer && <MobileTrailerModal src={mobileTrailer} title={selection.title} onClose={() => setMobileTrailer(null)}/>}</div>;
}
