...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><button className="brand" onClick={() => setPage("Home")}><i></i>CineCircle</button><p>MENU</p><nav>{nav.map((item, index) => <button key={item} className={shown === item ? "active" : ""} onClick={() => setPage(item)}><span>{["⌂", "⌕", "✦", "♧", "◉"][index]}</span>{item}{item === "For You" && <b>3</b>}</button>)}</nav><div className="account"><Avatar>SB</Avatar><div><strong>Shawn Baker</strong><span>Free plan</span></div></div></aside>
    <main><header><button className="mobile-brand" onClick={() => setPage("Home")}><i></i>CineCircle</button><label className="search">⌕<input placeholder="Search movies, shows, people..."/></label><div><button className="bell" aria-label="Notifications">♧</button>{recommend()}</div></header>
