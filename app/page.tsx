...
export default function Home() {
  ...
  const [modal, setModal] = useState<"recommend" | "groupPick" | null>(null);
  ...
  const recommend = () => <button className="primary recommend-action" onClick={() => { setPage("Discover"); flash("Open a title to recommend it to someone."); }}>+ Recommend</button>;
  ...
  return <div className="app-shell">
    ...
    {modal === "recommend" && shareTitle && <RecommendationModal title={shareTitle} onClose={() => setModal(null)} onSent={(name) => { setModal(null); flash(`Recommendation sent to ${name} ✦`); }} />}
