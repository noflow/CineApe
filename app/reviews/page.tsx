...
async function ReviewsPageLegacy() {
  const reviews = db ? await db.select({
    ...
    .where(and(eq(editorReviews.status, "published"), or(isNull(editorReviews.scheduledAt), lte(editorReviews.scheduledAt, new Date())))).orderBy(desc(editorReviews.publishedAt)).limit(48) : [];
